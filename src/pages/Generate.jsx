...
export default function Attribute() {
  ...
  const { getModels, error: modelsError } = useGenerator();
  const { generateImage, loading, error: generatorError } = useGenerator();
    ...
    const fetchModels = async () => {
      try {
        ...
        if (response.status === "success" && response.data.models.length > 0) {
          setModels(response.data.models);
          setSelectedModel(response.data.models[0]); // Set default ke model pertama
        } else {
          setError(modelsError || "Failed to fetch models");
        }
      } catch (err) {
  ...
  return (
    <div className="bg-background min-h-screen flex flex-col gap-y-4">
      <Header />
      <h1 className="text-4xl font-bold text-accent text-center">Generate Gambar</h1>
      ...
      <div className="flex items-center gap-x-4">
        <div className="w-full max-w-md mx-auto space-y-10">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 rounded-2xl shadow-2xl p-6">
            <div className="flex items-center justify-between gap-x-6">
              <label htmlFor="model" className="text-primary font-bold text-xl w-1/4">Model</label>
              <select
                id="model"
                name="model"
                className="p-2 border-2 border-secondary rounded-xl w-full focus:outline-none focus:border-primary text-accent font-semibold"
            ...
            <div className="flex items-center justify-between gap-x-6">
              <label htmlFor="seed" className="text-primary font-bold text-xl w-1/4">Seed</label>
              <input
                type="number"
                id="seed"
                name="seed"
        ...
        <div className="w-full max-w-md mx-auto space-y-6">
          ...
          {generatorError ? (
            ...
            <div className="flex flex-col items-center rounded-2xl shadow-2xl p-6 gap-y-6">
              ...
              <button onClick={handleDownload} className="bg-primary text-white font-bold py-2 px-4 rounded-xl hover:bg-accent">
                Download gambar
              </button>
            </div>
          )}
